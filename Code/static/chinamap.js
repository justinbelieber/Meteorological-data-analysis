    ...
    for (var state in china) {
        ...
        (function (st, state) {
            ...
            function clickMap() {
                ...
                if(china[state]['e_name'] !='xiang_gang' && china[state]['e_name'] !='tai_wan' && china[state]['e_name'] !='shanghai' && china[state]['e_name'] !='jiang_su' && china[state]['e_name'] !='gan_su')
                {
                   window.location.href="success2.html";
	               socket.emit('choosePlace', china[state]['e_name']);
                   alert("您选择了"+state);
                }
                else
                {
                   socket.emit('choosePlace', china[state]['e_name']);
                   alert("您选择的" + state + "仍属中国领土，但暂无数据");
                }
            }
        })
